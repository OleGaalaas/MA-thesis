# ============================================================
# 03_dependency_directed_sentiment_dropped.py
# Directed sentiment — DROPPED experts robustness check (Lenta)
#
# Runs the same directed sentiment pipeline on the ~37k expert
# mention rows that were excluded from the main analysis because
# no country could be attributed to them (all mention_country ==
# "Unknown").  Mirrors the "dropped" set computed in run_topic_model.R:
#
#   dropped = raw_mentions
#             |> filter(person != "Unknown")
#             |> anti_join(final_mentions, by = "row_id")
#
# country_final is set to "Unknown" and is_foreign_final to False
# for all rows (consistent with the reason they were dropped).
#
# Hybrid method:
...
#
# Inputs:
#   Data/Output/lenta_experts_trf_1999_2024_mentions.csv  (raw)
#   Data/Topic modelling/mentions_dataset_for_sentiment.csv  (kept)
#
# Outputs:
#   Data/lenta_dropped_sentiment_full.csv
#   Data/lenta_dropped_sentiment_long.csv
# ============================================================
...
RAW_MENTIONS_PATH = r"C:/Users/olemga/OneDrive - PRIO/Documents/MA Thesis/Methods/Data/Output/lenta_experts_trf_1999_2024_mentions.csv"
KEPT_MENTIONS_PATH = r"C:/Users/olemga/OneDrive - PRIO/Documents/MA Thesis/Methods/Data/Topic modelling/mentions_dataset_for_sentiment.csv"

OUTPUT_FULL_PATH = r"C:/Users/olemga/OneDrive - PRIO/Documents/MA Thesis/Methods/Data/lenta_dropped_sentiment_full.csv"
OUTPUT_LONG_PATH = r"C:/Users/olemga/OneDrive - PRIO/Documents/MA Thesis/Methods/Data/lenta_dropped_sentiment_long.csv"
...
# ------------------------------------------------------------
# LOAD DATA — derive dropped set (mirrors run_topic_model.R)
# ------------------------------------------------------------

print("Loading raw mentions...")
raw = pd.read_csv(RAW_MENTIONS_PATH)
print(f"  Raw mentions: {len(raw):,}")

print("Loading kept mentions (final dataset)...")
kept = pd.read_csv(KEPT_MENTIONS_PATH, usecols=["row_id"])
kept_row_ids = set(kept["row_id"].unique())
print(f"  Kept row_ids: {len(kept_row_ids):,}")

# Replicate R logic: person != "Unknown", row_id not in final dataset
df = raw[
    (raw["person"] != "Unknown") &
    (~raw["row_id"].isin(kept_row_ids))
].copy().reset_index(drop=True)

print(f"  Dropped rows (person known, no country): {len(df):,}")
print(f"  Dropped unique articles: {df['row_id'].nunique():,}")

# Add columns required by the pipeline — all Unknown/False since these
# experts were dropped precisely because country attribution failed
df["country_final"] = "Unknown"
df["is_foreign_final"] = False
...
print(f"Rows after sentence cleaning: {len(df):,}")
...
print("\n--- Negative rate by role class (dropped experts have unknown country) ---")
print(long_df.groupby(["role_class", "target_bloc"])["sentiment_score"].apply(lambda x: (x == -1).mean()))
